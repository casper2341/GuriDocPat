...
fun PatientDateBookingScreen(
    ...
    Box {
        Column(
            ...
            Text(
                "Select Date for Checking Availability",
            ...
            state.selectedDate?.let { date ->
                TimeSlotSelector(
                    ...
                    onNavigateToBookingScreen = { slot, date ->
                        val gson = Gson()
                        val dateJson = Uri.encode(gson.toJson(date))
                        val slotJson = Uri.encode(gson.toJson(slot))
                        navController.navigate("bookAppointment/$doctorId/${dateJson}/${slotJson}")
                    }
                )
            }
        }
...
fun TimeSlotSelector(
    ...
    onNavigateToBookingScreen: (TimeSlot, Date) -> Unit
) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
